module InputType
	...
	class BasicInput
		...
		# Update the state of the handler.  The set argument
		# specifies the collection in which to look for
		# the next trigger
		def update(set)
			next_state = if @active
							if set.include? @binding
	...
	class Action < BasicInput
		...
		def update
			set =	if @binding.is_a? Fixnum # aka Gosu::KbA.class
						@buttons
	...
	class Sequence < BasicInput
		...
		def update
			set =	if @trigger[@current].is_a? Fixnum # aka Gosu::KbA.class
						@buttons
					else # Assume this is the name of another event
						@inputs[:action]
					end
	...
	class Chord < BasicInput
		def initialize(inputs, buttons, trigger, functions={})
			super inputs, buttons, trigger, functions
		end
		
		def update
			prev = handler.last_input # needed so negative edge can be detected
			current = handler.current_input # needed to detect positive edge and active states
			
			if @active
				if @buttons.include? prev
					# Active and still active
					handler.active_trigger
				else
					# Falling edge
					handler.falling_edge_trigger
				end
			else
				if @buttons.include? current
					# Rising edge
					handler.rising_edge_trigger
				else
					# Nothing of importance
